...
import { ingredientsToDescription, instructionsToDescription } from "@/services/recipeFormat";
...
export default function CreateRecipeScreen() {
  ...
  const canSubmit = useMemo(() => {
    ...
    return title.trim().length > 0 && cleanIngredients.length > 0 && cleanSteps.length > 0 && !submitting;
  ...
  const onCreate = async () => {
    ...
    if (!cleanTitle || !cleanIngredients.length || !cleanSteps.length) {
      Alert.alert("Create Recipe", "Please complete name, ingredients, and instructions."); return;
    }
    try {
      setSubmitting(true);
      await createRecipe(cleanTitle, ingredientsToDescription(cleanIngredients), instructionsToDescription(cleanSteps));
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
        ...
        <Text style={[styles.h1, { fontFamily: fontBold, fontSize: 22 * finalScale }]}>Create Recipe</Text>
        <View style={styles.underline} />
        <Text style={[styles.introText, { fontFamily: fontRegular, fontSize: 14 * finalScale }]}>
        ...
        <View style={styles.formCard}>
          ...
          <TextInput value={title} onChangeText={setTitle}
            placeholder="e.g., Banana Yogurt Bowl" placeholderTextColor="#66706C"
            style={[styles.input, { fontFamily: fontRegular, fontSize: 15 * finalScale, minHeight: tt.minHeight }]}
            editable={!submitting} />

          <Text style={[styles.sectionTitle, { fontFamily: fontBold, fontSize: 16 * finalScale }]}>Ingredients</Text>
          <Text style={[styles.sectionHelper, { fontFamily: fontRegular, fontSize: 12 * finalScale }]}>
            List each ingredient separately for a cleaner recipe layout.
          </Text>
          <IngredientsInputList values={ingredients} onChange={setIngredients} disabled={submitting} />

          <Text style={[styles.sectionTitle, { fontFamily: fontBold, fontSize: 16 * finalScale }]}>Instructions</Text>
          <Text style={[styles.sectionHelper, { fontFamily: fontRegular, fontSize: 12 * finalScale }]}>
        ...
        <View style={styles.btnRow}>
          ...
          <Pressable onPress={onCreate}
            style={({ pressed }) => [styles.primaryBtn, { minHeight: tt.minHeight, borderRadius: tt.borderRadius }, (!canSubmit || submitting) && styles.primaryBtnDisabled, pressed && styles.pressed]}
            disabled={!canSubmit || submitting}>
            {submitting
              ? <View style={styles.submitRow}><ActivityIndicator color="#fff" /><Text style={[styles.primaryText, { fontFamily: fontBold, fontSize: 15 * finalScale }]}>Creating...</Text></View>
              : <Text style={[styles.primaryText, { fontFamily: fontBold, fontSize: 15 * finalScale }]}>Create Recipe</Text>}
          </Pressable>
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  formCard: { 
    ...
    padding: 16 },
  sectionTitle: { marginTop: 16, fontWeight: "900", color: "#17302C" },
  sectionHelper: { 
    marginTop: 6, 
    color: "rgba(14,29,27,0.58)", 
    lineHeight: 18, 
    fontWeight: "600" },
  label: { marginTop: 0, fontWeight: "800", color: "#17302C" },
  input: { 
    marginTop: 8, 
